...
* Must be written in Asciidoc in Acidoctor-flavor (with sections identified by the `#` characters, not the `=` one ) or Markdown.
* Must content a `README.adoc` or `README.md` introductionnal document at the root of the base template URL.
* Must content a `base-template-metadata.yaml` file at the root of the base template URL. An sample is provided bellow.
