...
- If a section does not reference a multi-value label, it is not token into account for matching.

TIP: Example: if the base template has a section labeled `["foo"]` but not `["level::..."]`, this section will be kept if `foo` is selected but even if all `level::...` labels are unselected.
