...
* `files_imported_into_templates` works the same way but targets the resulting full templates (the `template/` folder in the generated archive). Use `dest_dir: .` for the root of `template/`.
