...
* `template/` keeps the filtered views with their metadata marker lines intact.
* `blank-template/` contains the same views reduced to section titles and AsciiDoc attributes (body text removed).
...
* Preview the resulting template per view (blank mode)
* Store configs (ex: set of selected labels...) so I can reopen it later
* Keep all sections from the base template : just select none label