...
* A `README.adoc` or `README.md` from the base template is always included at the root of the archive.

If none `--include` labels are provided, the resulting template contains every base template sections. The base template **must** expose a `README.adoc` or `README.md` (case-insensitive) alongside `base-template-metadata.yaml`.
