...
labels:
  # Form 1 (snake_case): control order of values as shown in the UI
  - name: level
    available_values: [basic, intermediate, advanced]
  # Form 2 (camelCase): equivalent to available_values
  - name: project_size
...
- Use a standard IETF BCP‑47 language code (e.g., `en`, `fr`, `es`, `de`, `pt`, `it`).
- The “See also” text inserted for `link_to` is currently localized for: `en`, `fr`, `es`, `de`, `pt`, `it`. Any other value falls back to English.
...
** Multi-valued labels use the `name::value` form (e.g., `project_size::large`, `project_size::medium`, `project_size::small`). Values are discovered from usage across parts, and you can also define them (and their order) in the manifest under `labels.available_values` (or `availableValues`).
...
- Define label value sets in `base-template-manifest.yaml` to:
  - Make all values selectable in the UI even if not present in any section yet.
  - Control the display order of values in the UI.
- Two equivalent forms are supported for values: `available_values` (snake_case) or `availableValues` (camelCase).
- Example:
